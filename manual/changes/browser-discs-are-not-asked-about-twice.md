...
What that request answered -- how long the image is -- is now kept by the browser, under the
address the disc was named by. A launch that names the same discs opens them out of what it
kept and asks the server nothing at all, so the first thing that reaches the network is the
game's own first read. A
...
check costs the player nothing: a disc whose length is no longer the one that was kept is
reported, and the next launch reads it afresh and discards what it was holding for the old
one. A read the server refuses settles the same question on the spot, and leaves the disc
data alone when the server answers that the image is the length it always was.
