...
The answer differs by path. A guest missing a map marked `yes` will not ask for it: on a LAN it reports that it cannot play and signs off. A map left unmarked is transferred from the host instead.
