...
A name that matches no loaded house creates a house under that name and stores its position, which lies past the end of the multiplayable list. The LAN side box then opens with nothing selected, while the skirmish dialog clamps the position to its second entry. The invented house stays in the list for the rest of the run.
:::
