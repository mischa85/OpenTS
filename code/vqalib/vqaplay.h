...
// The values are the ones the player has always reported, so a caller that compares
// against an integer still gets the answer it expects. The type is what a function
// returns when an error code is the only thing it can report; VQA_H_FUNC keeps its
// intptr_t, because VQAEVENT_LOCK answers through it with a pointer.
