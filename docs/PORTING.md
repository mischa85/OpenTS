...
| 5. Platform backend | in progress — the SDL host runs the engine natively on macOS arm64: it boots from freeware game data into a campaign mission, with mouse positions mapped through the frame scaling, rendering through bgfx/Metal, and sound through the shared OpenAL backend over openal-soft; `OPENTS_DISCS` mounts 2048-byte ISO images the file layer reads from wherever the install falls short, which is what carries the Nod campaign, the movies, and the music; VQA movies play natively; Linux is still open |
